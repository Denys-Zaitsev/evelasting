...
function formatDate(value: string, language: "en" | "uk") {
  ...
  return new Intl.DateTimeFormat(language === "uk" ? "uk-UA" : "en", { year: "numeric", month: "short", day: "numeric" }).format(date);
}
      ...
      <div className="grid items-center gap-12 lg:grid-cols-[1fr_0.88fr] lg:gap-20">
        <AnimatePresence mode="wait">
          <motion.div key={`${trackKey}-cover`} initial={{ opacity: 0, x: -45, scale: 0.96 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: 30, scale: 0.98 }} transition={{ duration: 0.65, ease: [0.16, 1, 0.3, 1] }} className="relative">
            <motion.button type="button" onClick={requestPlay} aria-label={`${t("playTrack")}: ${track?.title || "Evelasting"}`} className="group relative block aspect-square w-full overflow-hidden rounded-[30px] border border-white/10 bg-neutral-950 text-left shadow-[0_45px_140px_rgba(0,0,0,0.7)]" whileHover={{ y: -8, rotate: 0.5, scale: 1.012 }}>
              {track?.artwork ? (
        ...
        <AnimatePresence mode="wait">
          <motion.div key={`${trackKey}-copy`} initial={{ opacity: 0, x: 45 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -25 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}>
            ...
            <p className="mt-7 text-[10px] font-semibold uppercase tracking-[0.35em] text-white/40">{t("nowPlaying")} · {formatDate(track?.publishedAt ?? "", language)}</p>
            ...
            <p className="mt-6 max-w-xl whitespace-pre-line text-sm leading-7 text-white/55 sm:text-base">{track?.description || t("releaseInformation")}</p>

            <div className="mt-9 grid grid-cols-2 gap-3">
              <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5 backdrop-blur-md"><p className="text-[9px] uppercase tracking-[0.3em] text-white/35">{t("published")}</p><p className="mt-3 text-lg font-medium text-white/90">{formatDate(track?.publishedAt ?? "", language)}</p></div>
